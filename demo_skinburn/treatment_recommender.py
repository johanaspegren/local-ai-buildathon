...
# CPU-only generation on a Pi is much slower than on a dev PC -- 60s was too
# tight in practice (every call was timing out on real Pi hardware), so this
# is deliberately generous. Still configurable via infer_pi.py's --llm-timeout.
DEFAULT_TIMEOUT = 180.0
# Hard cap on generated tokens so a single call can't run away well past the
# timeout budget even if the model ignores the "under 220 words" instruction.
MAX_OUTPUT_TOKENS = 400
# Keep the model loaded in memory between calls so repeated runs don't pay
# the (slow, on a Pi) model-load cost every time. "-1" = keep loaded
# indefinitely until Ollama is restarted or explicitly unloaded.
KEEP_ALIVE = "10m"
...
def get_recommendation(predicted_label: str, confidence: float,
    ...
    guidance (no LLM) if Ollama isn't reachable, times out, or returns
    something unusable.
    """
    payload = {
        "model": model,
    ...
    try:
        ...
        if isinstance(e, TimeoutError) or "timed out" in str(e).lower():
            e = TimeoutError(
                f"{e} -- generation took longer than {timeout:.0f}s. On a Pi this "
                f"usually means the model is CPU-bound and/or RAM is under "
                f"pressure (check `free -h` and `ollama ps` for swapping). Try "
                f"a larger --llm-timeout, or see the README troubleshooting section."
            )
